...
function getProductImage(variant: string): string | null {
  const map: { [key: string]: string } = {
    ...
    'Albert Toustový chléb máslový, balený': '/logos/alberttoast.png',
    ...
    'Albert Toustový chléb světlý, balený': '/logos/albertsvetlytoast.png',
  ...
  useEffect(() => {
    getShoppingList()
      ...
      .catch((err) => {
        ...
        setItems([]) // Show empty cart UI instead of error
      })
  }, [])
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[60vh] text-center text-gray-700">
          <img src="/logos/empty.png" alt="Empty cart" className="w-32 h-32 mb-4" />
          <h2 className="text-2xl font-semibold mb-2">Your basket is empty</h2>
          <p className="mb-4">
            Looks like you haven’t added any items yet. Start exploring to find the best deals!
          </p>
          <button
        ...
        <>
          ...
          {/* Best Deal Button */}
          {!dealsCalculated && !isLoadingDeals && (
